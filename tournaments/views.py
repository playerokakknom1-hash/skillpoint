...
def _tournaments_with_counts(qs):
    """Annotate tournament queryset with pre-computed team count."""
    return qs.annotate(_teams_count=Count('registered_teams'))


def home(request):
    tournaments = (
        _tournaments_with_counts(
            Tournament.objects.filter(status='registration')
        )
...
def tournaments_list(request):
    tournaments = (
        _tournaments_with_counts(Tournament.objects.all())
        .select_related('created_by')
        .order_by('-created_at')
    )
...
def tournament_detail(request, tournament_id):
    tournament = get_object_or_404(
        _tournaments_with_counts(Tournament.objects.select_related('created_by')),
        id=tournament_id,
    )
    matches = (
        tournament.matches
...
def create_team(request):
    if request.method == 'POST':
        ...
        Team.objects.create(name=team_name, captain=request.user)
...
def my_team(request):
    team = (
        Team.objects
        .filter(captain=request.user)
        .prefetch_related('members', 'tournaments')
        .first()
    )
    if team is None:
        member_team = (
        ...
        if member_team is not None:
            return render(request, 'tournaments/my_team.html', {'team': member_team})
...
def join_tournament(request, tournament_id):
    ...
    try:
        ...
        if tournament.registered_teams.filter(id=team.id).exists():
...
def leave_team(request):
    try:
        Team.objects.get(captain=request.user)
...
def friends_list(request):
    ...
    sent_requests = (
        ...
        .filter(from_user=user, is_accepted=False)
        .select_related('to_user')
    )
    received_requests = (
        FriendRequest.objects
        .filter(to_user=user, is_accepted=False)
    ...
    friend_ids = set(
        FriendRequest.objects
        .filter(
            models.Q(from_user=user, is_accepted=True) |
            models.Q(to_user=user, is_accepted=True)
        )
    ...
    for from_id, to_id in friend_ids:
        resolved_ids.add(from_id if from_id != user.id else to_id)
    resolved_ids.discard(user.id)

    friends = User.objects.filter(id__in=resolved_ids) if resolved_ids else User.objects.none()

    search_query = request.GET.get('search', '').strip()
    exclude_ids = resolved_ids | {user.id}
    all_users_qs = User.objects.exclude(id__in=exclude_ids)
    if search_query:
        all_users_qs = all_users_qs.filter(username__icontains=search_query)
    all_users = all_users_qs[:50]
...
def accept_friend_request(request, request_id):
    ...
    friend_req.save(update_fields=['is_accepted'])
